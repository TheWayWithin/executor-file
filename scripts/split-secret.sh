...
#
# Defaults: -t 2 -n 3 -w estate  (any 2 of 3 shares reconstruct the
# passphrase; any single share reveals nothing).
#
...
while getopts "t:n:w:h" opt; do
  case "$opt" in
    t) THRESHOLD="$OPTARG" ;;
    n) SHARES="$OPTARG" ;;
    w) TOKEN="$OPTARG" ;;
    h) sed -n '2,13p' "$0"; exit 0 ;;
    *) echo "usage: $0 [-t threshold] [-n shares] [-w token]" >&2; exit 1 ;;
  esac
done
...
if [ "$THRESHOLD" -gt "$SHARES" ]; then
  echo "error: threshold ($THRESHOLD) cannot exceed share count ($SHARES)." >&2
  exit 1
fi
